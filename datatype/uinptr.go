...
func GetUintptr() {
	...
	// | Type		| Size			| Range														|
	// | uintptr	| Platform dep	| Unsigned integer large enough to store a pointer address	|
	//
	// A uintptr is an unsigned integer type that is large enough-
	// to hold the raw bit pattern of any memory pointer.
	// On a 32-bit architecture, uintptr is 32 bits wide.
	// On a 64-bit architecture, it is 64 bits wide.
	//
	// While it sounds like a pointer,
	// it has one massive difference: uintptr is just a plain integer.
	// The Go runtime and the Garbage Collector (GC) do not recognize it as a pointer.
	//
	// uintptr is an unsigned integer that large enough
	// to store pointer, where its size depends of cpu architecture
	// (4 bytes for 32-bit system, and 8 bytes for 64-bit system).
	//
	// It's primarily used for low-level memory manipulation,
	...
	//
	// uintptr is an integer type capable of storing
	// the bit pattern of any pointer.
	// It's used in unsafe operations, converting unsafe.Pointer
	...
	n := 100
	ptr := unsafe.Add(unsafe.Pointer(&n), 8)
	addr := uintptr(ptr)
