...
const (
	...
	IS_MALE_TEMPLATE  string = "Is Male  : %t (%T)\n"
)
...
// 2. Using :=         : This is short variable declaration,
// where local variables which are declared and initialized in the functions.
//
...
// 2. := can only be used inside functions and not be allowed at package level.
// Variable declaration and value assignment cannot be done separately
// (must be done in the same line). It must initialize immediately.
//
...
func DeclareUsingVar() {
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale1, isMale1)      // Output: Is Male  : true (bool)

	// Below example, variables declared and initialized without the explicit type.
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale2, isMale2)      // Output: Is Male  : false (bool)

	// If the expression is removed, then the variable holds default value for the type-
	// like 0 for number, false for boolean, "" for string,
	// and nil for interface and reference type.
	// There is no such concept of an uninitialized variable.
	//
	...
	//
	// Below example, variables declared and initialized without expression.
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale3, isMale3)      // Output: Is Male  : false (bool)

	// If you use type, then you are allowed to declare-
	// multiple variables of the same type in the single declaration.
	// It's only possible to declare one type of variable per line.
	...
	// different types of variables on the same line.
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale4, isMale4)      // Output: Is Male  : true (bool)
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale5, isMale5)      // Output: Is Male  : false (bool)

	// You're allowed to initialize a set of variables-
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale6, isMale6)      // Output: Is Male  : false (bool)
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale7, isMale7)      // Output: Is Male  : true (bool)
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale8, isMale8)      // Output: Is Male  : true (bool)

	// It's possible to assign a value to a variable after it is declared.
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale9, isMale9)      // Output: Is Male  : false (bool)
	...
	var (
		...
		nickName10        = "Sarah"
		age10             = 17
		address10  string = "Flat 40, 7771 Victoria Street, Shopping Centre, Manchester, Wales, HG3 8ZT, United Kingdom"
		isMale10   bool
	)
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale10, isMale10)      // Output: Is Male  : false (bool)
}

func DeclareUsingShortVariableDeclaration() {
	// Most of the local variables are declared and initialized-
	// (at the same time) by using short variable declarations-
	// due to their brevity and flexibility.
	//
	// Don't confuse in between := and =,
	// as := is a declaration and initialization, and = is assignment.
	//
	// It's impossible to declare a variable using := without assigning a value to it.
	// It declares and initializes at the same time.
	//
	// Type is inferred automatically and it only works inside functions.
	// This is the Go-style you'll see everywhere.
	fullName1, nickName1, age1, address1, isMale1 := "Makayla Daugherty", "Makkie", 18, "Flat 24, 9260 Windsor Road, Leisure Complex, London, West Yorkshire, LU7 3BD, United Kingdom", false
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale1, isMale1)      // Output: Is Male  : false (bool)

	// Using short variable declaration you are allowed to-
	// declare multiple variables in the single declaration.
	...
	// You're allowed to declare multiple variables of-
	// different types in the single declaration.
	// The type of these variables are determined by the expression (inferred).
	...
	fmt.Printf(IS_MALE_TEMPLATE, isMale2, isMale2)      // Output: Is Male  : true (bool)
	...
	// for alias1 variable because same variable present in the same block-
	...
	//
	// a := 10
	// a := 20 // no new variables on left side of := compiler (NoNewVar)
	//
	// Syntax:
	//
	// a := 10
	...
	// alias1 := "Ivy Velasquez" // no new variables on left side of :=compiler (NoNewVar)
	//
	// Quick comparison between var and := (short variable declaration):
	// 1. Type inference       : var (Y) | := (Y)
	// 2. Declare without value: var (Y) | := (N)
	// 3. Package level        : var (Y) | := (N)
	// 4. Outside functions    : var (Y) | := (N)
	// 5. Inside functions     : var (Y) | := (Y)
	// 6. Idiomatic for local  : var (N) | := (Y)
